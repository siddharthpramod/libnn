...
%
% This is adapted from UFLDL exercise, modified to accept examples in rows
...
for d = 1:numel(stack)
    ...
    assert(size(stack{d}.w, 2) == size(stack{d}.b, 2), ...
        ['The bias should be a *row* vector of ' ...
         int2str(size(stack{d}.w, 2)) 'x1']);
    if d < numel(stack)
        assert(size(stack{d}.w, 2) == size(stack{d+1}.w, 1), ...
...
if nargout > 1
    ...
    if numel(stack) == 0
        ...
        netconfig.inputsize = size(stack{1}.w, 1);
        netconfig.layersizes = {};
        for d = 1:numel(stack)
            netconfig.layersizes = [netconfig.layersizes ; size(stack{d}.w,2)];
        end
    end
end
