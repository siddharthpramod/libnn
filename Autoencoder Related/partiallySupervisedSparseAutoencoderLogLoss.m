function [cost,grad] = partiallySupervisedSparseAutoencoderLogLoss(theta, visibleSize, hiddenSize, ...
                                             lambda, sparsityParam, beta, data, X, y, lambdaSup, betaSup)
%% Computes cost (reconstruction + prediction error) and gradient for one layer autoencoder
...
cost   = sum(sum(-data.*log(a3)-(1-data).*log(1-a3)))/m + lambda/2*(sum(sum(W1.^2))+sum(sum(W2.^2))) + beta*KL_penalty ...
    + betaSup*(sum(-y.*log(h)-(1-y).*log(1-h))/mSup + lambdaSup/2*(sum(W2Sup(2:end).^2)));

% log-loss error term + regularization + sparsity penalty ...
